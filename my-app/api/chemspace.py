...
def createChemicalSpace (smiles_df, smilesColumn):
    ...
    tsne = manifold.TSNE(n_components=2, init='random', random_state=42, metric='precomputed')
    ...
    coords = pd.concat([coorDice.reset_index(drop=True), coorCos.reset_index(drop=True)], axis=1, sort=False)
    coords = pd.concat([coords.reset_index(drop=True), coorTanimoto.reset_index(drop=True)], axis=1, sort=False)

    
    coords = pd.concat([coords.reset_index(drop=True), coorDiceUmap.reset_index(drop=True)], axis=1, sort=False)
    coords = pd.concat([coords.reset_index(drop=True), coorCosUmap.reset_index(drop=True)], axis=1, sort=False)
    coords = pd.concat([coords.reset_index(drop=True), coorTanimotoUmap.reset_index(drop=True)], axis=1, sort=False)
    
    
    # finally we concat the coordinates with the res dataframe containing the smiles
    
    res_df = pd.concat([res_df.reset_index(drop=True), coords.reset_index(drop=True)], axis=1, sort=False)
